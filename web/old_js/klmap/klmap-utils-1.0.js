...
function addPoint(pointLayer, x, y, id, name, popupFeature, clickCallback, iconWidth, iconHeight, iconPicUrl, iconOffsetX, iconOffsetY, isOffset, isHand,
	...
	if(iconPicUrl == null) iconPicUrl = 'js/klmap/point.png';
...
//获取地图的范围
function getBoundsLonlatNoOffset(){
	var jwd;
	var boundsLonlat = map.getExtent().toGeometry().transform(map.getProjectionObject(), map.displayProjection);
		boundsLonlat = boundsLonlat.components[0].components;
		if(boundsLonlat!=null && boundsLonlat.length>0){
			var pointList = new Array();
			for(var i=0;i<boundsLonlat.length;i++){
				pointList[i] = (boundsLonlat[i].x-offsetX)+","+(boundsLonlat[i].y-offsetY);
				if(i==0){
					
					jwd=boundsLonlat[i].x+","+boundsLonlat[i].y+",";
				}
				if(i==2){
					
					jwd=jwd+boundsLonlat[i].x+","+boundsLonlat[i].y;
				}
