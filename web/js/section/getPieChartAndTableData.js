/**
 * Created by pangbo on 2015/12/8.
 */
var serverUrl = "http://127.0.0.1:8080/";
//声明全局变量来保存表格数据
var globalTableData;

function getPieChartAndTableData()
{
    $.ajax({
        async:true,
        cache:false,
        type: "post",
        dataType : "json",
        url: serverUrl + "getSectionPageData.action",
        error: function () {
...
function showPieChart(rs)
{
    ...
    var strXML="<chart caption='路段拥堵指数' showValues='0' showZeroPies='1' manageLabelOverflow='1' showBorder='0' bgAlpha='10'  baseFont='宋体' baseFontSize='12' >";
    ...
    {
        values[chartData[j].SECTION_TPI_STATUS-1] = chartData[j].COUNTNUM;
    ...
    for(var i=0;i<5;i++){
        ...
        var label = labels[i] + ":" + values[i] + "个路段";
...
function showTable(rs)
{
    ...
    congestionStatus[0] = "非常畅通";
    congestionStatus[1] = "畅通";
    congestionStatus[2] = "轻微拥堵";
    congestionStatus[3] = "中度拥堵";
    congestionStatus[4] = "重度拥堵";

    for (var i = 0; i<tableData.length; i++)
    {
        var tableBody = "";
        tableBody += "<tr>";
        tableBody += "<td>" + tableData[i].SECTION_NAME + "</td>";
        tableBody += "<td>" + (tableData[i].SECTION_TPI).toFixed(1) + "</td>";
        tableBody += "<td>" + congestionStatus[tableData[i].SECTION_TPI_STATUS-1] + "</td>";
        tableBody += "</tr>";
        $("#sectionInfo").append(tableBody);
    }
...
function asc(para)
{
    ...
    for (var i = 0; i<globalTableData.length; i++)
    {
        var tableBody = "";
        tableBody += "<tr>";
        tableBody += "<td>" + globalTableData[i].SECTION_NAME + "</td>";
        tableBody += "<td>" + (globalTableData[i].SECTION_TPI).toFixed(1) + "</td>";
        tableBody += "<td>" + congestionStatus[globalTableData[i].SECTION_TPI_STATUS-1] + "</td>";
        tableBody += "</tr>";
        $("#sectionInfo").append(tableBody);
    }
}
