...
function initTable()
{
    $.ajax({
        ...
        type: "post",
        dataType: "json",
        url:"createWarnDetailTable.action?pageNumber=0&showPerPage=" + show_per_page
        +"&searchKey=" + encodeURI(encodeURI(searchKey))+"&beginTime="+beginTime+"&endTime="+endTime+"&deviceId="+deviceId,
        error:function(){
            alert("fail");
        },
...
function createTablePage(currentPageIndex,showPerPage)
{
    $.ajax({
        ...
        type:"post",
        dataType:"json",
        url:  "createWarnDetailTable.action?"+"orderKey="+theOrderKey+"&orderType="+theOrderType +
        "&pageNumber=" + currentPageIndex + "&showPerPage=" + show_per_page+"&searchKey=" + encodeURI(encodeURI(searchKey))+"&beginTime="+beginTime+"&endTime="+endTime+"&deviceId="+deviceId,
        error:function(){
            alert("sort fail");
        },
        success:function(result)
...
function orderINC(input)
{
    ...
    $.ajax({
        ...
        type:"post",
        dataType:"json",
        url:  "createWarnDetailTable.action?"+"orderKey="+orderBasedKey+"&orderType="+orderType +
        "&pageNumber=0&showPerPage=" + show_per_page+"&searchKey=" + encodeURI(encodeURI(searchKey))+"&beginTime="+beginTime+"&endTime="+endTime+"&deviceId="+deviceId ,
        error:function(){
            alert("sort fail");
        },
        success:function(result)
    ...
    try {
        ...
        for (var i = start; i < end; i++) {
            ...
            tbBody += "<tr class='" + trColor + "'>"
                + "<td>" + (('WARN_ID' in datas[i]) != false ? datas[i].WARN_ID : "--") + "</td>"
 ...
 */
function getBack(){
    window.location.href="/pages/warnCenter/warnStat.jsp?deviceId="+deviceId+"&beginTime="+showStartTime+"&endTime="+showEndTime+"&searchKey="+encodeURI(encodeURI(searchKey));
    //window.history.go(-1);
}